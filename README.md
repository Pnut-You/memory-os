...
短期摘要按 `user_id + device_id + Asia/Shanghai 日期` 隔离，使用 SQLite 原始对话作为事实源，不依赖 Redis 判断边界。

- 默认 `SUMMARY_EVERY_TURNS=10`、`SUMMARY_RETAIN_TURNS=5`。
- 同一天达到 10 轮完整对话时，生成或更新当天摘要；跨天重新从当天会话开始计算。
- 同一天继续增长时，每新增约 5 轮更新一次当天摘要。
- `conversation_summaries.local_date`、`from_event_id` 和 `to_event_id` 可用于确认摘要日期与压缩范围。
- 摘要正文不是无限拼接旧摘要；每次后台摘要只重写当天最近窗口摘要，摘要正文硬限制为约 1600 字符，更早的稳定偏好应进入 `user_preferences` 和 Redis 用户卡片。
