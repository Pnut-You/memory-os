...
Worker 调用 Qwen / DashScope OpenAI-compatible 服务后，模型输出必须经过结构校验和确定性合并，才能写入当前 `user_id + device_id` 的 `user_preferences` 和 `preference_evidence`。偏好变化后创建带 `device_id` 的 `user_card_rebuild` 任务，重建该设备的 Redis 用户卡片。
...
- Redis/SQLite 当前设备用户卡片。
- 当前 `user_id + device_id + session_id` 的滚动摘要。
...
- `profile.occupation` 是单值槽位。同一用户同一设备只保留一个 active 职业；如果用户说“我现在转行成医生”，旧的“程序员”会变成 `superseded`，新职业成为 active。
- `preference.likes` 是多值集合。同一用户同一设备可以同时 active “喜欢旅游”“喜欢摄影”“喜欢吃苹果”；重复表达同一对象只增加 `evidence_count`。
- `preference.dislikes` 是多值集合。同一用户同一设备可以同时 active “不喜欢香菜”“不喜欢吵闹”；重复表达同一对象只增加 `evidence_count`。
