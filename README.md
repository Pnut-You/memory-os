...
- 长期职业、喜欢和不喜欢归属 `user_id`，跨设备共享；其他设备相关偏好仍可带 `device_id`。
- 短期上下文归属 `user_id + device_id + session_id`。新请求到达时，如果距上次活动超过 15 秒，系统创建新 session；这不是后台心跳或定时关闭事件。
- 设备实时状态归属：`device_id`。
- 对外 API 不需要 start/end session 调用。当前没有“session 结束后自动触发长期偏好抽取”的机制。
...
- `user-card`：某个 `user_id + device_id` 的 L0 记忆卡片，包含用户级长期事实和适用的设备偏好，可从 SQLite active preferences 重建。
...
- `user_preferences`
  - 保存 L1 结构化用户偏好。职业、喜欢和不喜欢按 `user_id` 存储并跨设备共享；其他类型可以保留设备范围。
...
这两个条件只负责创建后台任务，并不在主请求中调用偏好模型。任务使用上一次成功处理位置和当前用户事件生成 `from_event_id/to_event_id`；当前实现不会等待 session 结束。
...
1. 按任务的 `user_id`、可选 `device_id` 和 `from_event_id < id <= to_event_id` 从 SQLite `events` 查询 `role='user'` 的原始消息。
2. 构造 `source_user_events`，每项只包含真实 `event_id`、用户原文和时间。
3. 程序先用轻量信号筛掉明显无关的消息。
4. 对每条候选用户原文分别调用一次模型；模型不会接收 Redis 卡片、滚动摘要、旧 preferences、assistant 文本或整个 session。
5. 系统校验模型输出和原文来源，通过后绑定真实 `event_id`，再执行确定性合并并写入 SQLite。

例如任务范围内有多条消息时，模型实际只会分别看到可能含偏好的单条原文：
...
- `profile.occupation` 是用户级单值槽位，新职业 supersede 旧职业。
...
`value` 必须逐字存在于当前原文，`evidence` 必须是当前原文中包含该值的句子；`none` 必须使用空 `value/evidence`。模型不生成数据库主键、scope、状态或合并动作，这些由程序确定。模型超时、非法 JSON、字段错误、空值或原文来源校验失败时不写入偏好；任务按配置的重试上限重试，不使用本地规则补写。

偏好变化后，系统清理该用户所有 Redis 用户卡片；后续读取可以从 SQLite active preferences 重建。职业、喜欢和不喜欢归属 `user_id`，因此可以跨设备使用。
